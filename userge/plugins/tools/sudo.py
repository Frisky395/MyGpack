...
async def sudo_(message: Message):
    ...
    if Config.SUDO_ENABLED:
        ...
        await message.edit("`Sudo Dimatikan !`", del_in=3)
    else:
        Config.SUDO_ENABLED = True
        await message.edit("`Sudo Dihidupkan !`", del_in=3)
...
async def add_sudo(message: Message):
    ...
    if user["id"] in Config.SUDO_USERS:
        await message.edit(f"user : `{user['id']}` Sudah Ada Di **SUDO**!", del_in=5)
    else:
        ...
        await asyncio.gather(
            ...
            message.edit(
                f"user : `{user['id']}` Ditambahkan Ke **SUDO**!",
                del_in=5,
                log=__name__,
            ),
        )
...
async def del_sudo(message: Message):
    ...
    if "-all" in message.flags:
        ...
        await asyncio.gather(
            ...
            message.edit("Menghapus Semua Pengguna **SUDO**", del_in=5),
        )
        return
    ...
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
    if not user_id:
        await message.err(f"user: `{user_id}` Tidak Ditemukan!")
        return
    if isinstance(user_id, str) and user_id.isdigit():
        user_id = int(user_id)
    ...
    if user_id not in Config.SUDO_USERS:
        await message.edit(f"user : `{user_id}` Tidak Di **SUDO**!", del_in=5)
    else:
        Config.SUDO_USERS.remove(user_id)
        await asyncio.gather(
            SUDO_USERS_COLLECTION.delete_one({"_id": user_id}),
            message.edit(
                f"user : `{user_id}` Dihapus Dari **SUDO**!", del_in=5, log=__name__
            ),
        )
...
async def view_sudo(message: Message):
    ...
    if not Config.SUDO_USERS:
        await message.edit("Tidak Menemukan Pengguna **SUDO**", del_in=5)
        return
    out_str = "😈 **PENGGUNA SUDO** 😈\n\n"
...
async def view_sudo_cmd(message: Message):
    ...
    if not Config.ALLOWED_COMMANDS:
        await message.edit("Perintah **SUDO** Tidak Ditemukan", del_in=5)
        return
    out_str = (
        f"🔱 **PERINTAH UNTUK SUDO** 🔱\n\n**trigger** : `{Config.SUDO_TRIGGER}`\n\n"
    )
